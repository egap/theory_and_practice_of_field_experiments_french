...
- **Problème fondamental de l'inférence causale** dans le framework contrefactuel. On ne peut pas observer à la fois $Y_i(1)$ et $Y_i(0)$ pour une unité donnée, donc on ne peut pas obtenir $\tau_i$ directement.
  Voir le module sur [l'inférence causale](causal-inference.html).
...
- **TODO Hypothèse nulle forte d'absence d'effet** L'hypothèse nulle selon laquelle il n'y a aucun effet du traitement pour aucun sujet.
...
- **Écart type** Racine carrée de l'écart quadratique moyen par rapport à la moyenne d'une variable. C'est une mesure de la dispersion ou de la propagation d'une statistique.
...
- **Puissance statistique d'un test** Probabilité qu'un test de causalité détectera un effet de traitement statistiquement significatif si l'effet existe.
  ...
  Cela dépend :
    ...
    - d'autres facteurs, y compris la proportion de vos unités qui sont affectées à différentes conditions de traitement.
...
- **Consistance d'un estimateur** Un estimateur qui produit des réponses qui se rapprochent de plus en plus de la vraie valeur de l'estimande à mesure que la taille de l'échantillon augmente est un *estimateur cohérent* de cette estimation. Un estimateur cohérent peut être sans biais ou non.
  Voir le module sur les [estimandes et estimateurs](estimands-and-estimateurs.html).
...
- **Terminé** Assigner $m$ sur $N$ unités au traitement. Vous savez combien d'unités seront traitées dans votre expérience et chaque unité a une probabilité de $m/N$ d'être traitée. Le nombre de façons dont le traitement peut être affecté (nombre de permutations d'assignation de traitement) est $\frac{N!}{m!(N-m)!}$.

- **Bloc** Divisez d'abord l'échantillon en blocs, puis randomisez séparément dans chaque bloc. Un bloc est un ensemble d'unités au sein desquelles vous effectuez une assignation aléatoire.

- **Grappe (Cluster)** Les groupes d'unités sont affectés au hasard aux conditions de traitement. Un cluster est un ensemble d'unités qui seront toujours affectées au même statut de traitement.

- **Combinaison Grappe et Bloc** Premièrement formez des blocs de clusters. Ensuite, dans chaque bloc, attribuez au hasard les grappes aux conditions de traitement en utilisant une randomisation complète.

## Designs factoriels

Voir le module sur [la randomisation](randomization.html).

- **Design factoriel** Un design avec plus d'un traitement, chaque traitement étant attribué indépendamment. Le plan factoriel le plus simple est un 2 par 2.

- **Effet marginal conditionnel** L'effet d'un traitement, conditionnel au maintien de l'autre pour une valeur fixe.
  Par exemple: $Y_i(T_1=1|T_2=0)-Y_i(T_1=0|T_2=0)$ est l'effet marginal de $T_1$ conditionnel à $T_2=0$.

- **Effet marginal moyen** Effet principal de chaque traitement dans un plan factoriel.
  C'est la moyenne des effets marginaux conditionnels pour toutes les conditions de l'autre traitement, pondérée par la proportion de l'échantillon qui a été affectée à chaque condition.

- **Effet d'interaction** Dans un plan factoriel, nous pouvons également estimer les effets d'interaction.
    - Pas d'effet d'interaction : un traitement n'amplifie ni ne diminue l'effet de l'autre traitement.
    - Effet d'interaction multiplicatif : l'effet d'un traitement dépend de la condition à laquelle une unité a été attribuée pour l'autre traitement.
...
- **Conformité** Le statut de traitement d'une unité correspond à la condition de traitement qui lui a été assignée.
   Exemple de non-conformité : une unité affectée au traitement ne le prend pas.
   Exemple d'observance : une unité affectée au contrôle ne prend pas de traitement.

- **Types de conformité** Il existe quatre types d'unités en termes de conformité :
     - **Conformistes** Unités qui prendraient un traitement si elles étaient affectées au traitement et qui ne seraient pas traitées si elles étaient affectées au contrôle.
     - **Toujours preneurs** Unités qui prendraient un traitement si elles étaient affectées au traitement et si elles étaient affectées au contrôle.
     - **Never-takers** Unités qui ne seraient pas traitées si elles étaient affectées au traitement et si elles étaient affectées au contrôle.
     - **Non-conformistes** Unités qui ne seraient pas traitées si elles étaient affectées au traitement et qui prendraient un traitement si elles étaient affectées au contrôle.
...
- **Design incitatif** Une expérience qui randomise $T$ (assignation de traitement), et nous mesurons $D$ (si l'unité prend le traitement) et $Y$ (le résultat).
  On peut estimer l'ITT et le LATE/CACE. Cela nécessite trois hypothèses.
     - **Monotonicité** Hypothèse qu'il n'y a soit pas de sujets non-conformistes ou de conformistes.
       Habituellement, nous supposons qu'il n'y a pas de non-conformistes, ce qui signifie que l'effet de l'affectation sur la prise du traitement est soit positif, soit nul mais pas négatif.
     - **Première étape** Hypothèse qu'il y a un effet de $T$ sur $D$.
     - **Restriction d'exclusion** Hypothèse selon laquelle $T$ affecte $Y$ uniquement jusqu'à $D$. C'est généralement l'hypothèse la plus problématique.

- **Effet d'intention de traiter (ITT)** L'effet de $T$ (assignation de traitement) sur $Y$.

- **Effet moyen local du traitement (LATE)** L'effet de $D$ (prise de traitement) sur $Y$ pour les conformistes.
