...
Pour profiter au maximum de ce livre, veuillez installer \href{https://cran.r-project.org/}{R} et \href{https://www.rstudio.com/products/rstudio/download/}{RStudio} sur votre machine. En fait, les slides supposent que vous utilisiez Rmarkdown pour les adapter à vos besoins.

Pour commencer avec R, voir le module \href{introduction-to-r-and-rstudio.html}{Introduction à R et RStudio}.

Vous pouvez copier ce livre ou des parties de celui-ci (des slides, etc.) soit en utilisant le bouton de téléchargement (Download) sur la première page de \url{http://github.com/egap/theory_and_practice_of_field_experiments_french}, soit directement sur github en ``forkant'' ce repertoire.
...
\item
  Introduction du package \href{http://declaredesign.org}{DeclareDesign} pour le design de recherche.
\item
...
\item
  \autocite{rubin:1974}. Rubin introduit l'idée de résultats potentiels et relie les conceptualisations contrefactuelles de la causalité à l'inférence statistique.
\end{itemize}
...
\item
  ...
  \begin{itemize}
  \item
    \textbf{Simple} : assigner de manière aléatoire des unités au traitement (comme un tirage au sort).
  \item
    \textbf{Complète} : au sein d'une liste d'unités éligibles, assigner un numéro fixe pour recevoir un traitement (comme un tirage d'une urne sans remise).
  \item
    \textbf{Par bloc (ou stratifié)} : assigner un traitement dans des strates ou des blocs spécifiques, comme si vous meniez une expérience dans chaque bloc.
  \item
    \textbf{En grappe (cluster)} : assigner des groupes (grappes ou clusters) à la même condition de traitement.
  \end{itemize}
\item
...
Les fichiers liés montrent comment \href{https://egap.github.io/learningdays-resources/Exercises/randomization-exercises.Rmd}{faire de la randomisation réplicable en R}.
Vous pouvez également voir plus d'exemples de randomisation en R sur \href{https://egap.org/resource/10-things-to-know-about-randomization/}{10 choses à savoir sur la randomisation}.
...
\emph{Randomisation en grappe (cluster)}
...
\emph{Randomisation en grappe par bloc}
...
\item
  \href{https://www.youtu.be/bL2U9z8hX1k}{Randomisation en grappe vs en bloc}
\end{itemize}
...
\item
  ...
  \item
    Évaluer le taux de faux positifs d'un test d'hypothèse pour un design donné et un choix de statistique de test;
...
\begin{itemize}
\item
  Guide des méthodes EGAP \href{https://egap.org/resource/10-things-to-know-about-hypothesis-testing/}{10 choses à savoir sur les tests d'hypothèse}
\item
...
\begin{itemize}
\item
  \autocite{gerber_field_2012}. Chapitre 3 : Distributions d'échantillonnage, inférence statistique et test d'hypothèse.
\item
...
\begin{itemize}
\item
  Un \textbf{effet causal}, \(\tau_i\), est une comparaison des résultats potentiels non observés pour chaque unité \(i\). Par exemple, cela peut être une différence ou un ratio de résultats potentiels non observés.
\item
...
\item
  Différentes randomisations produiront différentes valeurs du même estimateur ciblant le même estimande. \textbf{L'erreur standard} résume cette variabilité dans un estimateur.
\item
...
\item
  \textbf{Analyser en randomisant} dans le contexte de l'estimation signifie que (1) nos erreurs standards devraient mesurer la variabilité de la randomisation et (2) nos estimateurs devraient cibler des estimandes définis en termes de résultats potentiels.
\item
...
\begin{itemize}
\item
  \href{https://egap.github.io/learningdays-resources/Slides/Examples/threats-benin.pdf}{Présentation des problèmes de design aux EGAP Learning Days à l'African School of Economics, Abomey-Calavi, juin 2019 (first section reviews randomization designs)}
\item
...
\hypertarget{puissance-statistique-et-diagnosands-de-design}{%
\chapter{Puissance statistique et diagnosands de design}\label{puissance-statistique-et-diagnosands-de-design}}
...
\item
  \href{https://www.youtube.com/watch?v=U8fme1boEbE}{Les comités de revue éthique institutionnels aux US}
\item
...
\item
  \textbf{Problème fondamental de l'inférence causale} dans le framework contrefactuel. On ne peut pas observer à la fois \(Y_i(1)\) et \(Y_i(0)\) pour une unité donnée, donc on ne peut pas obtenir \(\tau_i\) directement.
  Voir le module sur \href{causal-inference.html}{l'inférence causale}.
\item
...
\item
  ...
  Notez qu'on n'utilise pas le style de notation \(E[Y_i (1)]\) ici parce que \(E[]\) signifie ``moyenne sur des opérations répétées,'' mais \(\overline{Y}\) signifie ``moyenne sur un ensemble d'observations''.
...
\item
  ...
  Il existe plusieurs stratégies d'assignation aléatoire : simple, complète, par grappe (cluster), par bloc, ou une combinaison bloc et grappe.
...
\item
  ...
  \item
    de la taille de l'effet (généralement mesurée en unités standardisées)
  \item
...
\item
  \textbf{Complète} Assigner \(m\) sur \(N\) unités au traitement. Vous savez combien d'unités seront traitées dans votre expérience et chaque unité a une probabilité de \(m/N\) d'être traitée. Le nombre de façons dont le traitement peut être assigné (nombre de permutations d'assignation de traitement) est \(\frac{N!}{m!(N-m)!}\).
\item
...
\item
  \textbf{Grappe (Cluster)} Les groupes d'unités sont assignés au hasard aux conditions de traitement. Un cluster est un ensemble d'unités qui seront toujours assignées au même statut de traitement.
\item
  \textbf{Combinaison Grappe et Bloc} Premièrement formez des blocs de clusters. Ensuite, dans chaque bloc, assignez au hasard les grappes aux conditions de traitement en utilisant une randomisation complète.
\end{itemize}
...
Au-delà des fonctions de base, nous pouvons également stocker des valeurs, des données et des fonctions dans l'environnement global. Pour affecter une valeur à une variable, utilisez l'opérateur \texttt{\textless{}-}. Toutes les valeurs, fonctions et données stockées apparaîtront dans l'onglet Environnement du fenêtre Gestion. Dans la Figure B.7, nous définissons la variable \texttt{t} pour prendre la valeur \(3 \times \frac{6}{14}\), et pouvons voir qu'elle est stockée sous Valeurs.
...
\begin{figure}
\includegraphics[width=0.6\linewidth]{Images/save_data} \caption{La valeur 3 * (6/14) est attribuée à la variable t (en rouge) et le dataset ChickWeight est ajouté à l'environnement global (en bleu).}\label{fig:savedata}
\end{figure}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{dim}\NormalTok{(ChickWeight) }\CommentTok{\# Dimensions du dataset dans l\textquotesingle{}ordre lignes puis colonnes}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{(b }\OtherTok{\textless{}{-}} \DecValTok{1}\SpecialCharTok{:}\DecValTok{10}\NormalTok{) }\CommentTok{\# ":"  est utilisé pour définir une chaîne d\textquotesingle{}entiers}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{(v }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{3}\NormalTok{, }\DecValTok{2}\NormalTok{, }\DecValTok{4}\NormalTok{, pi)) }\CommentTok{\# utilisez c() pour faire un vecteur avec n\textquotesingle{}importe quoi dedans}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Attribue une nouvelle valeur aux éléments particuliers d\textquotesingle{}un vecteur}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{which}\NormalTok{(b }\SpecialCharTok{\textless{}} \DecValTok{5}\NormalTok{) }\CommentTok{\# Donne les indices des éléments dont la valeur répond à la condition}
\end{Highlighting}
\end{Shaded}
